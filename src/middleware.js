...
export const processEvent = async (event, { bp, storage, logger, config }) => {
  ...
  if (config.qnaMakerApiKey) {
    ...
    event.qnaConfidence = answer.confidence
    if (!answer) {
      return false
    }
  ...
  if (!answer.enabled) {
    logger.debug('QnA: no matching question')
    event.reply(config.textRenderer, { text: config.qnaLowConfidenceAnswers })
    return true
  }

  if (answer.action.includes('text')) {

    console.log('QnA: minumum confidence: ', Number(config.qnaMinimumConfidence))
    console.log('QnA: answer confidence: ', answer.confidence );

    if( answer.confidence <= Number(config.qnaMinimumConfidence) ) {
      logger.debug('QnA: anwser confidence is too low')
      event.reply(config.textRenderer, { text: config.qnaLowConfidenceAnswers })
      return false
    }

    logger.debug('QnA: replying to recognized question with plain text answer', answer.id)
    event.reply(config.textRenderer, { text: `Q: ${answer.question}` })
